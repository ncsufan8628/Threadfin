...
var plexCategoriesOption = new Array("-", "Kids", "News", "Movie", "Series", "Sports")
  ...
  for (var i = 0; i < posFloat.length; i++) {
    ...
    if (allXmltvFiles.indexOf(item["x-xmltv-file"]) != -1) {
      ...
      switch (xXmltvFile) {
        ...
        case "Threadfin Dummy": newTD["_text"] = xXmltvFile; break;
    ...
    switch (keys[i]) {
      case "x-xmltv-file":
        ...
        for (var j = fileIDs.length - 1; j >= 0; j--) {
          if (fileIDs[j] != "Threadfin Dummy") {
            value.push(getValueFromProviderFile(fileIDs[j], "xmltv", "file.threadfin"))
    ...
    switch (key) {
      ...
      case "x-xmltv-file":
        ...
        switch (value) {
          ...
          case "Threadfin Dummy": element.innerHTML = value; break;
  ...
  if (bulk == false) {
    ...
    setTimeout(function () {
      ...
      switch (xXmltvFile) {
        ...
        case "Threadfin Dummy": dataFile.innerHTML = xXmltvFile; break;
...
function showXEPG() {
  var url = location.protocol + "//" + location.hostname + ":" + location.port + "/xmltv/threadfin.xml"
